...
const startServers = async () => {
  const ollamaAvailable =
    spawnSync("sh", ["-c", "command -v ollama"], {
      stdio: "ignore",
      env: process.env,
    }).status === 0;
  ...
  if (ollamaAvailable) {
    ...
    console.warn("Ollama binary not found. Skipping Ollama server startup.");
  }
  ...
  const exitCode = await Promise.race([
    ...
    ...(ollamaProcess ? [new Promise((resolve) => ollamaProcess.on("exit", resolve))] : []),
  ]);
